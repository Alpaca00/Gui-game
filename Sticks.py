...
for k, v in buttons.items():
    t.Button(frame2, text=k, width=20, style='Kim.TButton', command=lambda number=v: get_num(number)).pack(fill=BOTH, side=LEFT)

frame1=Frame(root, bg='#1F252A', bd=5)
frame1.pack(fill=X)

btn_start = t.Button(frame1, text='Start', style='Kim.TButton', command=start)
btn_start.grid(row=0, column=1, ipadx=27)

btn_res = t.Button(frame1, text='Restart', style='Kim.TButton' , command=restart)
btn_res.grid(row=0, column=2, ipadx=27)

btn_exit = t.Button(frame1, text='Exit', style='Kim.TButton', command=quit)
btn_exit.grid(row=0, column=3, ipadx=27)


label = Label(root, text='Tabulistan is proud to be an Official Sponsor.', font='Arial, 9')
label.pack()
